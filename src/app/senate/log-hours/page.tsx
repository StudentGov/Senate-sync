...
export default function SenateHourLoggingPage() {
  ...
  const [periodKey, setPeriodKey] = useState(() =>
    Math.floor(Date.now() / (14 * 24 * 60 * 60 * 1000))
  );
  ...
  async function submitHours(e?: React.FormEvent) {
    ...
    try {
      ...
      await fetch("/api/senate/log-hours", {
        method: "POST",
  ...
  function getChicagoParts(dt: Date) {
    ...
    const parts = fmt.formatToParts(dt).reduce((acc: any, p: any) => {
  ...
  function getChicagoOffsetMs(dt: Date) {
    ...
    const tzAbbrev =
      parts2.find((p: any) => p.type === "timeZoneName")?.value || "";
    ...
    <main className="min-h-screen bg-white">
      ...
      <div className="w-full">
        <div className="max-w-7xl mx-auto px-6 py-12 text-center">
          ...
          <p className="mt-2 text-purple-800/80">
            Track your contributions and stay accountable
          </p>
        </div>
      </div>
              ...
              <form onSubmit={submitHours} className="mt-8">
                <div className="space-y-4">
                  {segments.map((seg, idx) => (
                    <div
                      key={idx}
                      className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end bg-gray-50 p-3 rounded min-w-0"
                    >
                      <div className="md:col-span-4 min-w-0">
                        ...
                        <input
                          ...
                          value={seg.date}
                          onChange={(e) =>
                            updateSegment(idx, "date", e.target.value)
                          }
                          className="w-full mt-2 border rounded px-3 py-2 min-w-0"
                        />
                      </div>
                      ...
                      <div className="flex items-center justify-end md:col-span-2">
                        <div className="text-sm text-gray-700">
                          {Math.round(
                            (minutesBetween(seg.start, seg.end) / 60) * 100
                          ) / 100}{" "}
                          hrs
                        </div>
                        <button
                          ...
                          className="ml-3 p-1 text-red-600"
                  ...
                  <div className="flex items-center space-x-3">
                    <button
                      type="button"
                      onClick={addSegment}
                      className="inline-flex items-center gap-2 px-4 py-2 rounded-md border border-dashed text-purple-700"
                    >
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full mt-2 border rounded p-4 min-h-[120px]"
                ...
                <div className="mt-8 flex items-center gap-4">
                  <button
                    ...
                    className="flex-1 text-white py-3 rounded-md font-medium shadow"
                  >
                    {saving ? "Saving..." : "Submit Hours"}
                  </button>
                  <button
                    type="button"
                    className="px-6 py-3 rounded-md border-2 border-purple-700 text-purple-700 font-medium"
                  >
                    Cancel
                  </button>
                </div>
                  ...
                  return (
                    <>
                      ...
                      <div className="flex justify-between items-center bg-yellow-50 p-3 rounded">
                        <div>
                          <div className="text-sm text-gray-600">
                            Remaining (target 6 hrs)
                          </div>
                          <div className="text-xs text-gray-400">
                            After current draft: {remainingAfter} hrs
                          </div>
                        </div>
                        <div className="text-sm font-semibold">
                          {remaining} hrs
                      ...
                      <div className="mt-2 p-2 bg-indigo-50 rounded text-sm text-indigo-700">
                        ...
                        — ends {periodEnd} (Chicago)
                      </div>

                      <div className="text-sm text-gray-600">
                        Draft hours: {draft} hrs
                      </div>
                      <button
                        onClick={() => setShowReport(true)}
                        className="mt-2 w-full py-2 rounded-md border-2 border-yellow-500 text-yellow-700 font-medium"
                      >
                        View Full Report
                      </button>
                    </>
                        ...
                        <div className="max-h-64 overflow-y-auto">
                          {rows.map(([date, meta]) => (
                            ...
                            >
                              ...
                              <div className="text-sm font-semibold">
                                {Math.round(meta.total * 100) / 100} hrs
                              </div>
                            </div>
                          ))}
                        </div>
