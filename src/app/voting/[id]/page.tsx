...
export default async function VotingDetailPage({
    ...
    return (
      <main className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <div className="mt-4">
            <Link href="/voting" className="text-purple-700 font-medium">
              ← Back to votes
            </Link>
          </div>
        </div>
  ...
  let recorded: any[] = [];
  try {
    recorded = JSON.parse((await fs.readFile(votesPath, "utf-8")) || "[]");
  } catch (e) {
    recorded = [];
  }
  ...
  recorded.forEach((v) => {
    if (String(v.voteId) === id) {
      const k = String(v.optionId);
      optionCounts[k] = (optionCounts[k] || 0) + 1;
    }
  });

  const options: Option[] = (base.data || []).map((o: any) => ({
    id: o.id,
    label: o.label,
    ...
    <main className="min-h-screen bg-gray-50">
      <div className="max-w-3xl mx-auto py-16 px-6">
        <div className="flex items-center gap-4">
          <Link href="/voting" className="text-sm text-gray-600">
            ← Back
          </Link>
          <h1 className="text-3xl font-semibold text-gray-800">
            {(base as any).title ?? "Untitled vote"}
          </h1>
        </div>
