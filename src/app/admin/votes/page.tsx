...
export default function AdminVotesPage() {
  ...
  async function createVote(e: React.FormEvent) {
    ...
    if (options.length === 0)
      return setError("Provide at least one option label");
    if (!newTitle || !newTitle.trim()) return setError("Title is required");
    try {
      const res = await fetch("/api/admin/votes", {
        method: "POST",
        body: JSON.stringify({ options, title: newTitle, running: newRunning }),
  ...
  async function deleteVote(id: string) {
    type VoteEntry = { title?: string };
    const entry = voting[id] as VoteEntry | undefined;
  ...
  function startEdit(id: string) {
    setEditingId(id);
    type VoteEntry = { title?: string; running?: boolean; data: VoteData[] };
    const entry = voting[id] as VoteEntry;
    ...
    setEditingRunning(entry?.running ?? true);
  }
  ...
  async function saveEdit() {
    ...
    try {
      ...
      const data = editingOptions.map((o, idx) => ({
        id: o.id ?? idx,
        label: o.label,
        value: o.value ?? 0,
      }));
  ...
  return (
    <div className="p-6">
      <div className="flex items-center gap-4 mb-4">
        ...
        <Link
          href="/voting"
          className="ml-auto inline-flex items-center gap-2 px-3 py-1.5 bg-gray-100 text-gray-800 rounded-md border"
        >
          ← Back to votes
        </Link>
      </div>
          ...
          <div className="flex gap-2 items-center">
            ...
            <label className="flex items-center gap-2 ml-4">
              <input
                type="checkbox"
                checked={newRunning}
                onChange={(e) => setNewRunning(e.target.checked)}
              />
                ...
                <div className="flex justify-between items-center">
                  <div>
                    <div className="flex items-center gap-3">
                      <strong>
                        {(entry as { title?: string }).title ?? "Untitled vote"}
                      </strong>
                      {(entry as { running?: boolean }).running ? (
                    ...
                    <div className="text-sm text-gray-600">
                      ...
                      {entry.data
                        .map(
                          (d: VoteData) =>
                            `${d.label} (${
                              (d as { count?: number }).count ?? d.value ?? 0
                            })`
                        )
                        .join(", ")}
                    </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex gap-2 items-center">
                        ...
                        <label className="flex items-center gap-2 ml-4">
                          <input
                            type="checkbox"
                            checked={editingRunning}
                            onChange={(e) =>
                              setEditingRunning(e.target.checked)
                            }
                          />
                          <span className="text-sm">Running</span>
                        </label>
                      ...
                      {editingOptions.map((opt, idx) => (
                        <div key={idx} className="flex gap-2 items-center">
                          <input
                            ...
                            value={opt.label}
                            onChange={(e) =>
                              updateEditingOption(idx, e.target.value)
                            }
                          />
                          <input
